...
def get_assistant_name() -> str:
    """Return the configured assistant name, or 'LUNA' if not set."""
    return load_api_keys().get("assistant_name", "LUNA") or "LUNA"
...
def save_assistant_config(assistant_name: str, user_name: str) -> None:
    ...
    if CONFIG_FILE.exists():
        try:
            ...
            data = {}
    data["assistant_name"] = assistant_name.strip() or "LUNA"
    data["user_name"] = user_name.strip()
    CONFIG_FILE.write_text(json.dumps(data, indent=4), encoding="utf-8")


# ── Assistant voice ──────────────────────────────────────────────────────────
# Gemini Live prebuilt voices. Kore (sweet, elegant, youthful girl) and Aoede (warm female).
# Names are proper nouns — identical in every language.
AVAILABLE_VOICES = ["Kore", "Aoede", "Puck", "Charon", "Fenrir"]
DEFAULT_VOICE    = "Kore"
...
def get_wake_word_enabled() -> bool:
    """Whether local wake-word gating is on (assistant sleeps until 'Hey Luna')."""
