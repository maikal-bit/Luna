"""
Drop-in LUNA plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — LUNA discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    player: the LunaUI instance — use player.write_log(f"LUNA: ...") to log,
    ...
    if player:
        try:
            player.write_log(f"LUNA: {result_text}")
        except Exception:
            pass
    return result_text
