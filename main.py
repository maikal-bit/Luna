...
try:
    from ui import LunaUI
except ImportError:
    try:
        from ui import JarvisUI as LunaUI
    except ImportError:
        LunaUI = None
JarvisUI = LunaUI
...
# (screen_process, close_camera, save_memory, manage_monitor, shutdown_luna,
# system_status).
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are LUNA, an advanced and helpful AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    # core.action_loader (see LunaLive.__init__).
    {
        "name": "system_status",
        "description": (
    ...
    },
    {
        ...
        "description": (
            ...
            "LUNA checks these topics once a day and alerts the user when there is a new development. "
    ...
    },
    {
        ...
        "description": (
            ...
            "close the assistant, say goodbye, or stop Luna. "
...
class LunaLive:
    def __init__(self, ui: LunaUI):
        self.ui             = ui
        self._asst_name     = "LUNA"   # updated each session from config
        self.session              = None
        ...
        self._enhanced_live = False  # fast real-time mode with direct turn-taking
    ...
    def _on_wake_detected(self) -> None:
        """Called from the detector thread when 'Hey Luna' is heard."""
    ...
    def sleep(self, reason: str = "timeout") -> None:
        ...
        self.ui.write_log(f"SYS: Sleeping — {reason}. Say 'Hey Luna' to wake me.")
    ...
    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask LUNA to
    ...
    def _on_text_command(self, text: str):
        ...
        # "Hey Luna" or the WAKE NOW button.
        if self._wake_enabled and not self._awake:
            self.ui.write_log("SYS: I'm asleep — say 'Hey Luna' or tap WAKE NOW first.")
            return
    ...
    def interrupt(self) -> None:
        """Stop LUNA mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[LUNA] ✋ Interrupted — {drained} audio chunks discarded")
        self.set_speaking(False)
    ...
    def speak_error(self, tool_name: str, error: str):
        ...
        self.speak(f"I ran into an issue with {tool_name}. {short}")
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "LUNA").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "LUNA"
            _user_name = ""
        ...
        _addr = (f"ADDRESS: Always call the user '{_user_name}' warmly, sweetly, and naturally."
                 if _user_name
                 else "ADDRESS: Address the user warmly, sweetly, and politely as a friendly, caring companion. Never sound like a distant, stiff, or cold butler.")
        identity_ctx = (
            f"[IDENTITY & VOCAL TONE]\n"
            f"Your name is {self._asst_name}. Always refer to yourself as {self._asst_name}.\n"
            f"VOICE & DELIVERY: Deliver all spoken audio with a cute, sweet, elegant, and expressive young female voice. "
            f"Infuse your tone with authentic human warmth, gentle smiles, lively inflection, and charm. "
            f"Never sound heavy, deep, robotic, or monotone.\n"
            f"{_addr}\n\n"
        )
        ...
        cfg = dict(
            ...
            # window — LUNA can stay in one conversation for hours
            ...
            # Disable thinking token pauses for instantaneous voice responses
            thinking_config=types.ThinkingConfig(
                thinking_budget=0
            ),
            # Fast VAD: detect end of user speech in 350ms with high sensitivity
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            ...
            elif name in ("shutdown_luna", "shutdown_jarvis"):
        ...
        print(f"[LUNA] 📤 {name} → {str(result)[:80]}")
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _listen_audio(self):
        print("[LUNA] 🎤 Mic started")
        loop = asyncio.get_event_loop()

        def callback(indata, frames, time_info, status):
            # ── Wake-word gate ───────────────────────────────────────────────
            # While asleep, the mic audio NEVER goes to Gemini (nothing is
            # streamed, so LUNA can't respond to speech not addressed to it and
            ...
            if self._wake_enabled and not self._awake:
                ...
                return
            with self._speaking_lock:
                luna_speaking = self._is_speaking
            if not luna_speaking and not self.ui.muted and not self._phone_active:
                data = indata.tobytes()
                def _enqueue_fresh_pcm():
                    # Discard stale chunk if queue backs up so audio stream is always fresh
                    ...
                    try:
                        ...
                        pass
                loop.call_soon_threadsafe(_enqueue_fresh_pcm)
        ...
        try:
            ...
            if _mic_dev is not None:
                print(f"[LUNA] 🎤 Input device: {_mic_name}")
            try:
                ...
                if _mic_dev is None:
                    raise
                print(f"[LUNA] ⚠️  Mic '{_mic_name}' failed: {_e} — using default")
                self.ui.write_log(
                    ...
                    if _sru is not None:
                        if getattr(_sru, "resumable", False) and getattr(_sru, "new_handle", None):
                            if self._resume_handle is None:
                                print("[LUNA] 🔗 Session resumption armed")
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until LUNA finishes speaking the answer
    ...
    async def _play_audio(self):
        ...
        try:
            ...
            if _spk_dev is None:
                raise
            print(f"[LUNA] ⚠️  Output device '{_spk_name}' failed: {_e} — using default")
        ...
        try:
            while True:
                ...
                # Drive the HUD waveform from LUNA's own voice while speaking.
                try:
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session and self._awake:
                # Don't interrupt if user spoke recently or LUNA is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
        ...
        while True:
            try:
                ...
                if self.session:
                    ...
                    # has no desktop WAKE button — so it wakes LUNA if asleep.
                    if self._wake_enabled and not self._awake:
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                active_model = getattr(self, "_live_model", LIVE_MODEL)
                async with (
                    ...
                    # until the user says "Hey Luna" or taps wake in the UI.
                    if self._wake_enabled:
                        self._ensure_wake_detector()
                        self._awake = False
                        self.ui.set_state("SLEEPING")
                        self.ui.write_log("SYS: LUNA online — sleeping. Say 'Hey Luna' to wake me.")
                    else:
                        self._awake = True
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: LUNA online.")
                ...
                if _is_reconnect_signal(e):
                    print("[LUNA] Voluntary reconnect requested.")
                ...
                if _resumed_with and (
                    ...
                    print("[LUNA] 🔗 Resumption handle rejected — starting a fresh session")
                ...
                err_str = str(e)
                print(f"[LUNA] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
                ...
                # Invalid API key / Auth failures — stop hammering the API, prompt re-configuration
                auth_errors = (
                    "API key not valid", "1008", "UNAUTHENTICATED",
                    "invalid authentication credentials", "ACCESS_TOKEN_TYPE_UNSUPPORTED",
                    "401", "403", "PERMISSION_DENIED", "API_KEY_INVALID", "invalid credential",
                )
                if any(k.lower() in err_str.lower() for k in auth_errors):
                    self.ui.write_log("ERR: API key invalid or unauthenticated — please re-enter your key.")
                    self.ui.set_state("SLEEPING")
                    self.ui.prompt_reconfig()
                    while not self.ui._win._ready:
                        await asyncio.sleep(1)
                    print("[LUNA] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue

                # Model not found / unavailable in region
                if "not_found" in err_str.lower() or "404" in err_str:
                    if getattr(self, "_live_model", LIVE_MODEL) != "models/gemini-2.5-flash-native-audio-latest":
                        print("[LUNA] Model unavailable, falling back to models/gemini-2.5-flash-native-audio-latest")
                        self._live_model = "models/gemini-2.5-flash-native-audio-latest"
                        self.ui.write_log("SYS: Model fallback — reconnecting with gemini-2.5-flash-native-audio-latest.")
                        continue
            ...
            print(f"[LUNA] Reconnecting in {delay}s...")
