...
def _summarize_with_gemini(transcript: str, video_url: str) -> str:
    ...
    response  = _client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=(
                "You are LUNA, an intelligent, articulate, and realistic female AI assistant. "
                "Summarize YouTube video transcripts clearly and concisely. "
                "Structure: 1-sentence overview, then 3-5 key points. "
                "Be direct, warm, and insightful. "
...
def _handle_play(parameters: dict, player) -> str:
    ...
    # Check if native YouTube Desktop is installed
    try:
        from actions.search_in_app import _find_youtube_desktop_binary, search_in_app as _search_in_app_fn
        if _find_youtube_desktop_binary():
            print(f"[YouTube] Desktop app detected, playing via YouTube Desktop app: {query}")
            return _search_in_app_fn(
                parameters={"app_name": "youtube", "query": query, "play_first": True},
                player=player
            )
    except Exception as e:
        print(f"[YouTube] In-app search fallback error: {e}")
...
def _handle_search(parameters: dict, player) -> str:
    query = parameters.get("query", "").strip()
    if not query:
        return "Please tell me what you'd like to search for on YouTube."

    try:
        from actions.search_in_app import search_in_app as _search_in_app_fn
        return _search_in_app_fn(
            parameters={"app_name": "youtube", "query": query, "play_first": False},
            player=player
        )
    except Exception as e:
...
TOOL = {
    "name": "youtube_video",
    "description": "Controls YouTube and YouTube Desktop app. Use for: playing videos, searching videos on YouTube app, summarizing a video's content, getting video info, or showing trending videos.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "action": {
                "type": "STRING",
                "description": "play | search | summarize | get_info | trending (default: play)"
            },
            "query": {
                "type": "STRING",
                "description": "Search query or video title to play or search for"
            },
            "save": {
                "type": "BOOLEAN",
