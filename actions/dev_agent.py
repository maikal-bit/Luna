...
PROJECTS_DIR     = Path.home() / "Desktop" / "LunaProjects"
MAX_FIX_ATTEMPTS = 5
...
def _build_project(
    ...
    proj_name    = project_name or plan.get("project_name", "luna_project")
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        if not _has_error(last_output, run_command):
            msg = (
                f"Project '{proj_name}' is working wonderfully! "
    ...
    msg = (
        ...
        f"Project is saved at {project_dir} — open it in VSCode to check manually."
    )
    if speak: speak(msg)
    return f"{msg}\n\nLast error:\n{last_output[:600]}"
