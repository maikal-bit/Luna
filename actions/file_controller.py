...
_DANGEROUS_UNIX_ROOTS = [
    Path("/bin"), Path("/sbin"), Path("/usr"), Path("/etc"), Path("/System"),
    Path("/Library"), Path("/proc"), Path("/sys"), Path("/dev"), Path("/boot")
]

def _is_safe_path(target: Path) -> bool:
    """Checks that target does not touch critical operating system files or drive roots."""
    try:
        resolved = target.resolve()
        # Prevent wiping entire drive root (e.g. C:\ or /)
        if resolved.parent == resolved:
            return False

        if _OS == "Windows":
            for danger in _DANGEROUS_WINDOWS_ROOTS:
                try:
                    if resolved == danger or resolved.is_relative_to(danger):
                        return False
                except Exception:
                    pass
            return True
        else:
            for danger in _DANGEROUS_UNIX_ROOTS:
                try:
                    if resolved == danger or resolved.is_relative_to(danger):
                        return False
                except Exception:
                    pass
            safe_unix = [Path.home(), Path.cwd(), Path("/tmp"), Path("/mnt"), Path("/media")]
            return any(resolved == r.resolve() or resolved.is_relative_to(r.resolve()) for r in safe_unix)
    except Exception:
        return False
