...
_APP_ALIASES: dict[str, dict[str, str]] = {
    ...
    "youtube":            {"Windows": "youtube desktop",         "Darwin": "YouTube",              "Linux": "youtube"},
    "youtube desktop":    {"Windows": "youtube desktop",         "Darwin": "YouTube",              "Linux": "youtube"},
    "ytdesktop":          {"Windows": "youtube desktop",         "Darwin": "YouTube",              "Linux": "youtube"},
}
...
def _find_windows_shortcut(name: str) -> str | None:
    """Finds a matching .lnk shortcut in Windows Start Menu directories."""
    import os
...
def _launch_windows(app_name: str) -> bool:
    ...
    # 1. Direct path check for YouTube Desktop
    if any(k in clean_name for k in ["youtube", "ytdesktop"]):
        user_prof = Path(os.environ.get("USERPROFILE", ""))
        yt_exe = user_prof / "YouTube Desktop" / "ytdesktop.exe"
        if yt_exe.exists():
            try:
                if hasattr(os, "startfile"):
                    os.startfile(str(yt_exe))
                else:
                    subprocess.Popen([str(yt_exe)])
                time.sleep(1.5)
                return True
            except Exception as e:
                print(f"[open_app] Launching YouTube Desktop failed: {e}")

    # 2. Check Start Menu .lnk shortcuts
    lnk = _find_windows_shortcut(clean_name)
    if lnk:
        try:
            if hasattr(os, "startfile"):
                os.startfile(lnk)
            ...
            print(f"[open_app] Launching shortcut '{lnk}' failed: {e}")

    # 3. Check shutil.which
    ...
    # 4. Check URI protocol schemes (e.g. spotify:, discord:, ms-settings:)
    if ":" in app_name:
        try:
            if hasattr(os, "startfile"):
    ...
    # 5. Bring to front if already running
    try:
        ps_cmd = f'(New-Object -ComObject WScript.Shell).AppActivate("{app_name}")'
        res = subprocess.run(
