...
def _schedule_mac(hour: int, minute: int) -> str:
    ...
    plist_path  = plist_dir / "com.luna.gameupdater.plist"
    (plist_dir / "com.jarvis.gameupdater.plist").unlink(missing_ok=True)
    script_path = Path(__file__).resolve()
    plist_content = f"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
  "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
    <key>Label</key><string>com.luna.gameupdater</string>
...
def _cancel_scheduled_update() -> str:
    if is_windows():
        r1 = subprocess.run(
            ["schtasks", "/Delete", "/TN", "LUNA_GameUpdater", "/F"],
            capture_output=True, text=True, **_CNW
        )
        r2 = subprocess.run(
        ...
        return ("Scheduled update cancelled."
                if (r1.returncode == 0 or r2.returncode == 0) else "No scheduled update found.")
    if is_mac():
        p1 = Path.home() / "Library" / "LaunchAgents" / "com.luna.gameupdater.plist"
        p2 = Path.home() / "Library" / "LaunchAgents" / "com.jarvis.gameupdater.plist"
        cancelled = False
        for plist_path in (p1, p2):
    ...
    try:
        existing = subprocess.run(["crontab", "-l"], capture_output=True, text=True)
        lines    = [l for l in existing.stdout.splitlines()
                    if "LUNA_GameUpdater" not in l and "JARVIS_GameUpdater" not in l]
...
def _get_schedule_status() -> str:
    if is_windows():
        for tn in ("LUNA_GameUpdater", "JARVIS_GameUpdater"):
    ...
    try:
        ...
        if "LUNA_GameUpdater" in result.stdout or "JARVIS_GameUpdater" in result.stdout:
            for line in result.stdout.splitlines():
                if "LUNA_GameUpdater" in line or "JARVIS_GameUpdater" in line:
