...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".luna_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".luna_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using LUNA profile")
                luna = str(Path.home() / ".luna_profiles" / "firefox_luna")
                Path(luna).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(luna, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".luna_profiles" / "safari")
        ...
        # automation). Fall back to a persistent LUNA automation profile —
        # accounts logged in here once stay logged in on later sessions too.
        luna_profile = str(Path.home() / ".luna_profiles" / self.browser_name)
        Path(luna_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with LUNA profile: {luna_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(luna_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with LUNA profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "luna_screenshot.png")
