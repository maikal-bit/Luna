"""
Local wake-word detection for LUNA ("Hey Luna").
...
CPU. The pretrained wake phrase used here is "Hey Luna" (detected via the pretrained model).
"""
...
class WakeWordDetector:
    ...
    def start(self) -> bool:
        ...
        self._thread.start()
        self._logger("Wake word: listening for 'Hey Luna'.")
        return True
        ...
        while self._running:
            try:
                ...
                if isinstance(scores, dict):
                    # Match luna or fallback jarvis model regardless of exact key suffix
                    for k, v in scores.items():
                        kl = k.lower()
                        if "luna" in kl or "jarvis" in kl:
