"""
Text-to-Speech engines for LUNA.
...
def create_tts_player(config: dict) -> TTSPlayer:
    ...
    if engine_name == "kokoro":
        voice  = config.get("tts_voice", "af_bella")  # Bella (cute youthful girl)
        speed  = float(config.get("tts_speed", 1.0))
        engine = KokoroTTSEngine(voice=voice, speed=speed)
    elif engine_name == "elevenlabs":
        api_key  = config.get("elevenlabs_api_key", "")
        voice_id = config.get("tts_voice", "EXAVITQu4vr4xnSDxMaL")  # Bella (cute, sweet, elegant girl)
        engine   = ElevenLabsTTSEngine(api_key=api_key, voice_id=voice_id)
    else:   # edgetts (default)
        voice  = config.get("tts_voice", "en-US-AnaNeural")  # Ana (sweet, cute, youthful girl)
