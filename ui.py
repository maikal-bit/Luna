...
class HudCanvas(QWidget):
    """
    3D Holographic Neon Particle Orb — inspired by ethereal glowing spherical plasma.
    Renders a dynamic 3D Fibonacci spherical particle cloud that rotates in 3D,
    tilts interactively with mouse movements, undulates with audio amplitude waves,
    and is encased within a translucent iridescent glass sphere rim (cyan -> magenta -> coral).
    """
    def __init__(self, face_path: str = "", assistant_name: str = "LUNA", parent=None):
        ...
        # 3D Rotation and Interactive Mouse Tilt
        self._rot_yaw    = 0.0
        self._rot_pitch  = 0.22
        self._rot_roll   = 0.05
        self._target_tilt_x = 0.0
        self._target_tilt_y = 0.0
        self._cur_tilt_x    = 0.0
        self._cur_tilt_y    = 0.0

        # Generate 3D Fibonacci Sphere Lattice (outer shell + inner depth points)
        self._sphere_points = []
        n_outer = 580
        for i in range(n_outer):
            y = 1.0 - (i / float(n_outer - 1)) * 2.0
            ...
            self._sphere_points.append([x, y, z, 1.0, theta, phi, 0])

        # Inner volume core points for rich translucent 3D optical density
        n_inner = 140
        for i in range(n_inner):
            y = 1.0 - (i / float(n_inner - 1)) * 2.0
            r_y = math.sqrt(max(0.0, 1.0 - y * y))
            theta = i * 2.399963229728653
            r_core = 0.65
            x = math.cos(theta) * r_y * r_core
            z = math.sin(theta) * r_y * r_core
            phi = math.acos(max(-1.0, min(1.0, y)))
            self._sphere_points.append([x, y * r_core, z, r_core, theta, phi, 1])

        # Ethereal floating aura particles
        self._aura_particles: list[list[float]] = []
    ...
    def mouseMoveEvent(self, e):
        ...
        cx, cy = W / 2.0, H / 2.0
        dx = (e.position().x() - cx) / max(100.0, cx)
        dy = (e.position().y() - cy) / max(100.0, cy)
        self._target_tilt_x = max(-0.45, min(0.45, dx * 0.42))
        self._target_tilt_y = max(-0.45, min(0.45, -dy * 0.42))
    ...
    def _step(self):
        ...
        # Decay audio amplitude smoothly
        decay = math.exp(-dt * 6.5)
        self._live_amp *= decay
        disp_decay = math.exp(-dt * 14.0)
        if self._live_amp > self._amp_disp:
            self._amp_disp = self._live_amp
        else:
            self._amp_disp += (self._live_amp - self._amp_disp) * (1.0 - disp_decay)

        amp = self._amp_disp

        # Smooth breathing & audio expansion
        breath = math.sin(self._tick * 0.04) * 0.02
        if self.muted:
            self._tgt_scale = 0.94
            self._tgt_halo  = 30.0
        elif self.speaking:
            self._tgt_scale = 1.0 + breath + amp * 0.15
            self._tgt_halo  = 75.0 + amp * 110.0
        else:
            self._tgt_scale = 1.0 + breath + amp * 0.08
            self._tgt_halo  = 55.0 + amp * 60.0

        sp = 0.35 if self.speaking else 0.20
        self._scale += (self._tgt_scale - self._scale) * sp
        self._halo  += (self._tgt_halo  - self._halo)  * sp

        # Smooth interactive mouse tilt
        self._cur_tilt_x += (self._target_tilt_x - self._cur_tilt_x) * 0.10
        self._cur_tilt_y += (self._target_tilt_y - self._cur_tilt_y) * 0.10

        # Continuous 3D rotation
        rot_spd = (0.012 + amp * 0.035) * (1.8 if self.speaking else 1.0)
        self._rot_yaw   = (self._rot_yaw + rot_spd) % (2.0 * math.pi)
        self._rot_pitch = (self._rot_pitch + rot_spd * 0.35) % (2.0 * math.pi)
        self._rot_roll  = (self._rot_roll + rot_spd * 0.15) % (2.0 * math.pi)

        # Spawn ethereal aura sparks when active or speaking
        if (self.speaking or amp > 0.08) and random.random() < 0.30:
            cx, cy = self.width() / 2.0, self.height() / 2.0
            fw = min(self.width(), self.height())
            ang = random.uniform(0, 2.0 * math.pi)
            dist = fw * 0.30 * self._scale
            self._aura_particles.append([
                ...
                1.0,
                random.choice(["#d946ef", "#00f0ff", "#ff6b4a"])
            ])

        self._aura_particles = [
            [p[0]+p[2], p[1]+p[3], p[2]*0.96, p[3]*0.96, p[4]-0.025, p[5]]
            for p in self._aura_particles if p[4] > 0
        ]
    ...
    def paintEvent(self, _):
        ...
        cx, cy = W / 2.0, H / 2.0
        fw = min(W, H)
        R = fw * 0.27 * self._scale
        amp = self._amp_disp
        time_v = self._tick * 0.06

        # ── 1. Deep Space Cosmic Radial Glow (Ethereal Bloom) ───────────────
        bloom_grad = QRadialGradient(cx, cy, R * 1.6)
        bloom_grad.setColorAt(0.0, QColor(140, 20, 190, int(min(60, self._halo * 0.6))))
        bloom_grad.setColorAt(0.4, QColor(0, 180, 230, int(min(40, self._halo * 0.4))))
        bloom_grad.setColorAt(0.7, QColor(255, 90, 60, int(min(25, self._halo * 0.25))))
        bloom_grad.setColorAt(1.0, QColor(3, 1, 7, 0))
        p.setBrush(QBrush(bloom_grad))
        p.setPen(Qt.PenStyle.NoPen)
        p.drawEllipse(QRectF(cx - R * 1.6, cy - R * 1.6, R * 3.2, R * 3.2))

        # ── 2. 3D Rotation Matrix Calculations ──────────────────────────────
        total_pitch = self._rot_pitch + self._cur_tilt_y
        total_yaw   = self._rot_yaw
        total_roll  = self._rot_roll + self._cur_tilt_x

        cyaw, syaw     = math.cos(total_yaw),   math.sin(total_yaw)
        cpitch, spitch = math.cos(total_pitch), math.sin(total_pitch)
        croll, sroll   = math.cos(total_roll),  math.sin(total_roll)

        # ── 3. Rotate & Project 3D Sphere Points ────────────────────────────
        cam_dist = R * 3.4
        projected = []

        for pt in self._sphere_points:
            x, y, z, base_r, theta, phi, is_core = pt
            wave = math.sin(phi * 4.0 + theta * 3.0 + time_v * 3.2) * (0.15 * amp + 0.02)
            cur_r = R * base_r * (1.0 + wave)
            ...
            persp = cam_dist / (cam_dist - Z)
            sx = cx + X * persp
            sy = cy + Y * persp

            norm_z = max(0.0, min(1.0, (Z / (R * 1.1) + 1.0) * 0.5))
            pt_sz = max(1.1, (1.2 + norm_z * 2.8) * persp * (0.8 if is_core else 1.0))

            ny = Y / R  # -1.0 (top) to +1.0 (bottom)
            nx = X / R  # -1.0 (left) to +1.0 (right)

            if ny > 0.15:
                # Bottom hemisphere: coral orange / warm red-orange
                coral_t = min(1.0, (ny - 0.15) / 0.85)
                r_c = 255
                g_c = int(70 + (1.0 - coral_t) * 40)
                b_c = int(60 * (1.0 - coral_t))
            elif nx < -0.15:
                # Top-left quadrant: electric cyan
                cyan_t = min(1.0, (-nx - 0.15) / 0.85)
                r_c = int(30 * (1.0 - cyan_t))
                g_c = int(210 + cyan_t * 35)
                b_c = 255
            else:
                # Top-right / interior: neon magenta / violet
                r_c = 217
                g_c = 50
                b_c = 239

            base_a = 45 if is_core else 55
            alpha = int(max(30, min(255, base_a + norm_z * 195)))
            if self.muted:
                alpha = int(alpha * 0.4)

            projected.append((Z, sx, sy, pt_sz, QColor(r_c, g_c, b_c, alpha)))

        # Depth Sort: back particles rendered first, front particles last
        projected.sort(key=lambda item: item[0])

        # ── 4. Render 3D Sphere Particles ───────────────────────────────────
        p.setPen(Qt.PenStyle.NoPen)
        for _, sx, sy, pt_sz, col in projected:
            p.setBrush(QBrush(col))
            p.drawEllipse(QPointF(sx, sy), pt_sz, pt_sz)

        # ── 5. Translucent Glass Shell & Iridescent Caustic Rim ─────────────
        glass_rect = QRectF(cx - R, cy - R, R * 2.0, R * 2.0)

        # Outer Iridescent Rim (conical gradient ring)
        rim_grad = QConicalGradient(cx, cy, 210.0)
        rim_grad.setColorAt(0.00, QColor(0, 240, 255, 230))    # Electric Cyan
        rim_grad.setColorAt(0.25, QColor(217, 70, 239, 240))  # Vivid Neon Magenta
        rim_grad.setColorAt(0.45, QColor(255, 42, 133, 230))  # Hot Pink
        rim_grad.setColorAt(0.70, QColor(255, 107, 74, 240))  # Warm Coral Orange
        rim_grad.setColorAt(0.90, QColor(251, 146, 60, 220))  # Amber
        rim_grad.setColorAt(1.00, QColor(0, 240, 255, 230))   # Cyan closure

        # Outer neon glow rim
        p.setBrush(Qt.BrushStyle.NoBrush)
        p.setPen(QPen(QBrush(rim_grad), 3.5))
        p.drawEllipse(glass_rect)

        # Soft ethereal outer halo rim
        p.setPen(QPen(QColor(217, 70, 239, 50), 9.0))
        p.drawEllipse(QRectF(cx - R - 3, cy - R - 3, (R + 3) * 2.0, (R + 3) * 2.0))

        # Upper-Left Specular Refraction Arc (Glass reflection crescent)
        spec_pen = QPen(QColor(0, 240, 255, 160), 2.2)
        p.setPen(spec_pen)
        p.drawArc(QRectF(cx - R * 0.94, cy - R * 0.94, R * 1.88, R * 1.88), int(95 * 16), int(75 * 16))

        # Bottom Coral Specular Caustic Arc
        coral_spec = QPen(QColor(255, 107, 74, 150), 2.0)
        p.setPen(coral_spec)
        p.drawArc(QRectF(cx - R * 0.93, cy - R * 0.93, R * 1.86, R * 1.86), int(230 * 16), int(60 * 16))

        # ── 6. Floating Aura Sparks ─────────────────────────────────────────
        ...
        # ── 7. Status Indicator & Hologram Text ──────────────────────────────
        sy = cy + R + 28
        if self.muted:
            ...
            sym = "●" if self._blink else "○"
            txt, col = f"{sym}  LISTENING", qcol(C.CYAN)
        else:
            sym = "●" if self._blink else "○"
        ...
        for i in range(N):
            if self.muted:
                ...
                shimmer = 0.55 + 0.45 * math.sin(self._tick * 0.20 + i * 0.65)
                idle    = 3.0 + 2.0 * math.sin(self._tick * 0.08 + i * 0.55)
                hgt     = int(max(2, min(26, idle + amp * 24.0 * env * shimmer)))
                frac    = i / float(N - 1)
                if frac < 0.5:
                    t = frac * 2.0
                    r = int(0 * (1 - t) + 217 * t)
                    ...
                    g = int(70 * (1 - t) + 107 * t)
                    b = int(239 * (1 - t) + 74 * t)
                cl = QColor(r, g, b, 230 if amp > 0.04 else 120)
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "luna"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("luna:") or tl.startswith("jarvis:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for LUNA", str(Path.home()),
            "All Files (*.*);;"
...
class _DropCanvas(QWidget):
    ...
    def paintEvent(self, _):
        ...
        bg_col = qcol("#1a0830" if z._drag_over else ("#120622" if z._hovering else C.PANEL))
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure LUNA before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
        ...
        self._key_input.setPlaceholderText("Paste your Gemini API key…")
    ...
    def _submit(self):
        ...
        if not key or len(key) < 20:
            self._key_input.setStyleSheet(
                ...
                f"border: 1px solid {C.RED}; border-radius: 3px; padding: 4px 8px; }}"
            )
            self._key_input.setPlaceholderText("Please enter your Gemini API key…")
            return
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="LUNA", user_name="",
        ...
        lay.addSpacing(4)
        lay.addWidget(_lbl("YOUR NAME  (leave blank for auto)", 8,
    ...
    def _save(self):
        name = self._name_input.text().strip() or "LUNA"
...
class AudioDeviceOverlay(_HudOverlay):
    """Choose which microphone LUNA listens to and which speakers it uses.
    ...
    is plugged in. 'LUNA can't hear me' is usually 'LUNA is listening to the
    webcam'."""
    ...
    def __init__(self, parent=None):
        ...
        self._in_box  = _row("MICROPHONE — what LUNA hears you with",
                             "input", get_input_device())
        lay.addSpacing(4)
        self._out_box = _row("SPEAKERS — what LUNA talks through",
...
class MemoryOverlay(_HudOverlay):
    """Everything LUNA has stored about you, and when it learned it.
    ...
    def _rebuild(self):
        ...
        hdr = QLabel("🧠  WHAT LUNA REMEMBERS")
        hdr.setFont(QFont("Courier New", 12, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.PRI}; background: transparent;")
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Luna actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — LUNA ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "LUNA").strip()
        ...
        self.on_interrupt      = None   # callable: () -> None — stop LUNA mid-speech
        self.on_voice_change   = None   # callable: () -> None — rebuild session with new voice
        self.on_audio_device_change = None  # callable: () -> None — reopen audio streams
        self._confirm_overlay  = None   # live ConfirmBanner, if one is on screen
        self.get_plugins       = None   # callable: () -> list[dict], set by LunaLive
        self.get_plugin_settings = None # callable: () -> list[dict] settings schemas, set by LunaLive
        self.on_wake_toggle    = None   # callable: (enable: bool) -> str, set by LunaLive
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_luna_icon(out_path: Path) -> bool:
        """
        Render a LUNA core/reactor icon at 4× resolution and downsample
    ...
    def _create_desktop_shortcut(self):
        ...
        # Core icon (.ico — also exported as .png for Linux/macOS)
        ico_path = Path(__file__).resolve().parent / "config" / "luna.ico"
        if not ico_path.exists():
            legacy_ico = Path(__file__).resolve().parent / "config" / "jarvis.ico"
            if legacy_ico.exists():
                ico_path = legacy_ico
            else:
                self._build_luna_icon(ico_path)
        ...
        try:
            ...
            if _os == "Windows":
                ...
                lnk_name = f"{self._assistant_name.upper()}.lnk" if self._assistant_name else "LUNA.lnk"
                lnk      = str(desktop / lnk_name)
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                app_name = f"{self._assistant_name.upper()}.app" if self._assistant_name else "LUNA.app"
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '<string>com.luna.assistant</string>\n'
    ...
    def _build_header(self) -> QWidget:
        ...
        _sub_text = ("Logical Universal Neural Assistant"
                     if _disp in ("LUNA", "L.U.N.A")
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        self._bar_net = MetricBar("NET", C.CYAN)
        self._bar_gpu = MetricBar("GPU", C.ACC2)
        self._bar_tmp = MetricBar("TMP", "#ec4899")
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._interrupt_btn.setStyleSheet(f"""
            QPushButton {{
                background: #18030d; color: {C.MUTED_C};
                border: 1px solid {C.MUTED_C}; border-radius: 4px;
            }}
            QPushButton:hover {{
                background: #2a0518; border: 1px solid #ff4d79; color: #ffffff;
            }}
            QPushButton:pressed {{
                background: #3d0722;
            }}
    ...
    def _build_input_row(self) -> QHBoxLayout:
        ...
        self._input.setPlaceholderText("Type a command or question for LUNA…")
        self._input.setFont(QFont("Courier New", 9))
        self._input.setFixedHeight(34)
        self._input.setStyleSheet(f"""
            QLineEdit {{
                background: #090416; color: {C.WHITE};
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("LUNA AI", C.PRI_DIM))
        return w
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    try:
                        winreg.QueryValueEx(key, "LUNA_AI")
                        return True
                ...
                return ((Path.home() / "Library" / "LaunchAgents" / "com.luna.assistant.plist").exists()
                        or (Path.home() / "Library" / "LaunchAgents" / "com.jarvis.assistant.plist").exists())
            else:
                return ((Path.home() / ".config" / "autostart" / "luna.desktop").exists()
                        or (Path.home() / ".config" / "autostart" / "jarvis.desktop").exists())
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    for val_name in ("LUNA_AI", "JARVIS_AI"):
                        try:
                            winreg.DeleteValue(reg, val_name)
                        except FileNotFoundError:
                            pass
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "LUNA_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.luna.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.luna.assistant</string>\n'
    ...
    def _wake_state(self) -> dict:
        ...
        if self.wake_get_state:
            try:
                ...
                pass
        # Before LunaLive has wired its callback (drawer built at startup).
    ...
    def _toggle_wake_word(self):
        ...
        if not st["ready"]:
            ...
            return
        # Already downloaded → just flip enabled/disabled through LunaLive.
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "LUNA") or "LUNA",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = "",
        ...
        self._assistant_name = name.strip() or "LUNA"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — {APP_VERSION}")
        self._title_lbl.setText(display)
        if display in ("LUNA", "L.U.N.A"):
            self._sub_lbl.setText("Logical Universal Neural Assistant")
    ...
    def _style_mute_btn(self):
        if self._muted:
            ...
            self._mute_btn.setStyleSheet(f"""
                QPushButton {{
                    background: #20040f; color: {C.MUTED_C};
                    border: 1px solid {C.MUTED_C}; border-radius: 4px;
                }}
                QPushButton:hover {{ background: #2e0616; }}
            """)
        else:
            self._mute_btn.setText("🎙  MICROPHONE ACTIVE")
            self._mute_btn.setStyleSheet(f"""
                QPushButton {{
                    background: #09041a; color: {C.CYAN};
                    border: 1px solid {C.BORDER_B}; border-radius: 4px;
                }}
                QPushButton:hover {{
                    background: #14082c; border-color: {C.CYAN}; color: {C.WHITE};
                }}
            """)
    ...
    def _check_config(self) -> bool:
        ...
        try:
            ...
            key = (d.get("gemini_api_key") or "").strip()
            if not key or len(key) < 20:
                return False
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "LUNA") or "LUNA"
