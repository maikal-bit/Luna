"""
dashboard/server.py — LUNA Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "LUNA Uploads",
        Path.home() / "Documents" / "LUNA Uploads",
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'LUNA-DASHBOARD-v1'
_AES_SALT_LEGACY = b'JARVIS-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"LUNA Dashboard Port {port}"
        prog_rule  = "LUNA Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="luna_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until LUNA is run as Administrator.")
...
class DashboardServer:
    ...
    @staticmethod
    def _ssl_certs() -> tuple[Path, Path]:
        certs = BASE_DIR / "config" / "certs"
        if (certs / "luna.key").exists() and (certs / "luna.crt").exists():
            return (certs / "luna.key", certs / "luna.crt")
        return (certs / "jarvis.key", certs / "jarvis.crt")

    @staticmethod
    def _ssl_enabled() -> bool:
        ssl_key, ssl_cert = DashboardServer._ssl_certs()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in LUNA to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('luna_token','{tok}');
  sessionStorage.setItem('jarvis_token','{tok}');
  sessionStorage.setItem('luna_key','{key}');
  sessionStorage.setItem('jarvis_key','{key}');
  localStorage.setItem('luna_device_token','{dev_tok}');
  localStorage.setItem('jarvis_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to LUNA…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key, ssl_cert = self._ssl_certs()
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in LUNA UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
